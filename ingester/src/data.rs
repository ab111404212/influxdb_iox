...
impl NamespaceData {
    ...
    async fn progress(&self) -> SequencerProgress {
        ...
        // if we are still actively buffering the last write (max
        // persisting number), mark as not buffered
        progress.actively_buffering(*self.buffering_sequence_number.read())
    }
}
